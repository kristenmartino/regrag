...
Used by the structured-output synthesis path: the model emits a list of
(claim, chunk_id, supporting_quote) triples; this module verifies that the
quote is genuinely present in the chunk (with normalization for whitespace,
smart quotes, and minor punctuation differences). Claims with bogus quotes
are dropped before the answer is rendered.

This is the structural counterpart to the LLM-as-judge approach in
substantive.py: instead of asking a judge whether the chunk supports the
claim, the model is forced to identify the supporting phrase up front, and
we check that the phrase actually exists.
"""
