...
This is what makes regulatory RAG different from general-purpose RAG. A consumer chatbot can hedge, paraphrase, or hallucinate a plausible-sounding answer and the cost is a confused user. A compliance analyst building toward a regulatory filing cannot use a system whose citations look authoritative but don't actually support the claim — the failure mode there is not embarrassment, it's a regulatory exposure. So the bar is different: every claim has to be tied to a specific passage, every passage has to substantively support the claim, and the system has to know when to refuse rather than answer.

The obvious alternatives don't clear that bar:

- **Plain prompt-with-document** (paste the order into Claude, ask the question) works for one document but breaks on cross-doc synthesis, doesn't scale past a couple of orders worth of context, and produces no audit trail.
- **A general-purpose RAG library with default settings** (LlamaIndex out of the box, vanilla pgvector) gets you retrieval but no verification — citations are best-effort and unfaithful in roughly 30% of cases per our baseline measurement (see [eval-results.md](eval-results.md) §"Baseline comparison"). The agentic decomposition and runtime LLM-as-judge that RegRAG adds are what move citation faithfulness from ~81% to 95% and refusal accuracy from 71% to 90%.
- **A fine-tuned domain model** could improve answer quality on FERC-specific terminology, but doesn't address the citation problem (the model still might or might not faithfully cite) and is much harder to update when the corpus grows.
- **Closed-vendor regulatory tools** exist for legal research, but they're opaque about retrieval and impossible to audit at the chunk level — a non-starter when the deployment context requires showing your work.

This project is a working demonstration of the regulatory-RAG bar. It ingests FERC orders, indexes them with hybrid vector and keyword search, and uses a LangGraph orchestration layer to decompose multi-document questions into independently retrievable sub-queries. Every answer carries citations to specific passages; every interaction is logged with the retrieved chunks, the prompt sent, and the model used. The current corpus is **17 FERC documents spanning 2003–2024**, covering the rulemaking arcs that matter most for the personas in §2: interconnection (Orders 2003 → 845 → 845-A → 2023 → 2023-A), transmission planning (Orders 1000 → RM21-17 ANOPR → 1920), distributed energy and storage (Orders 745 → 841 → 841-A → 2222 → 2222-A → 872), and market mechanics (Order 825). Order 841 is held in both its FERC-issued errata form and its Federal Register publication — the two carry different effective-date prose (the FERC-issued text uses an `[INSERT DATE 90 DAYS AFTER PUBLICATION]` placeholder; the Federal Register version has the literal "June 4, 2018"), which matters for citation faithfulness on date-specific queries. The case study below walks through the architecture, the engineering tradeoffs, and what would need to change for a production deployment in a federal context.
...
**Cross-document misattribution** is a third, subtler failure the two-step pipeline above does *not* catch, and it took a specific fix. Consider "what was the effective date of Order 841?" The corpus contains Order 845-A (the rehearing of a *different* order) whose text discusses effective-date mechanics in language that is genuinely on point — so a chunk from 845-A passes both the structural check (it's a real chunk) and the substantive judge (it really does talk about 90-day effective windows). The model would cite it, and a reader clicking the citation would land in the wrong order. The substantive judge can't catch this because the chunk *does* support the claim; it's just the wrong *source* for it. The fix has three parts: (1) **document-anchored retrieval** — when a query names a specific order, a separate retrieval pass restricted to that order's accession number(s) is fused into the candidate pool, so chunks *from* the named order are guaranteed representation even when chunks *about* it from other documents have higher raw similarity; (2) a **per-sentence scope instruction** in the synthesis prompt requiring the first citation in any sentence about the named order to come from that order's own documents; and (3) a **post-generation scope verifier** (no LLM call — pure rule) that strips out-of-scope first-citations or, if too many sentences violate, signals regeneration. On the canonical Order 841 query, this moved the cited source from Order 845-A to Order 841's own Final Rule, and — once the Federal Register publication of Order 841 was added to the corpus and parsed with column-aware extraction — surfaced the literal effective date ("June 4, 2018") with a citation to the document that actually states it.
...
**Current results** (v6, 40 questions, 17-doc corpus, voyage-3.5-lite + claude-sonnet-4-6 + claude-haiku-4-5):

| Metric | Score | Notes |
|---|---|---|
| Retrieval recall | **95.8%** | Macro-averaged over answer-expected questions |
| Refusal accuracy | **92.5%** | 37/40 — 3 borderline questions on the answer/refuse boundary |
| Refusal precision / recall | **77.8% / 87.5%** | Of refusals emitted, how many were correct / of should-refuse questions, how many were caught |
| Citation faithfulness | **94.8%** | LLM-as-judge over cited claims, after the runtime substantive-support filter described in §5 stripped misattributed citations |

The citation-faithfulness number reflects the post-§5 cleaned answer. The pre-filter baseline (chunk-id verification only, no substantive judge) was **70.3%** on the original 28-question set; turning on the inline judge moved that to 91.2%, and growing the eval to the full corpus settled it in the mid-90s (95.4% at v5, 94.8% at v6 — the small v5→v6 move is within the non-deterministic judge's run-to-run noise). The architecture generalized cleanly to new content — see [docs/eval-results.md](eval-results.md) for the per-persona breakdown, the v5→v6 delta with two honestly-accounted question-level wrinkles, the baseline comparison, and a "what didn't work" section on the structured-output verification experiment that lost to the LLM judge.

Per-persona breakdown at v6 shows single-doc compliance questions are now the strongest (refusal 100%, recall 100%, faithfulness 98%) — doc-anchored retrieval helps single-document obligation lookups most — while cross-order counsel questions are the hardest on recall (87.5%, pulled down by one question whose answer depends on a court opinion not in the corpus). The full per-question report including judge rationale lives in [docs/eval-results.md](eval-results.md) and the raw `packages/eval/results/` JSON.
...
**Hybrid versus pure-vector retrieval.** Pure vector search is designed to miss queries that hinge on exact identifiers — "Order 2222," docket number RM18-9, statutory references — because the embedding space treats those tokens as low-information. The hybrid approach combines top-20 vector similarity with top-10 keyword matches over a 'simple'-tokenizer GIN index (preserves modal verbs and identifiers that English stemming would strip), fused via reciprocal rank fusion (k=60). An additional identifier-match recall floor guarantees that any chunk containing an exact identifier from the query is included in results regardless of vector or keyword score. A document-anchored retrieval step (added after the v5 review) restricts a separate top-K vector search to the canonical accession(s) of any order named in the query, fixing the failure mode where a chunk that *references* Order N outranks chunks *from* Order N itself. Across the full 40-question eval, retrieval recall is **95.8%** — the corpus reliably surfaces the chunks that contain the answer. **Measured (issue #5):** an isolated retrieval-only A/B — `hybrid_retrieve` vs pure `_pure_vector_topk`, same keyword-recall scoring — found hybrid does **not** beat pure vector on this eval: 94.8% vs 96.9% recall over the 32 answer questions, identical on 31 of them. On this small, semantically-rich corpus, vector search already pulls the right document into the top-10, so the identifier floor has nothing left to recover; the eval has essentially one pure docket-style query, too thin to show a difference. So the earlier “beats pure vector” framing is dropped: the identifier floor is cheap insurance that shows no measurable recall gain here (keyword-recall can't see ranking gains, which is a separate untested question). This is distinct from the **doc-anchored retrieval** win in §5 (the Order 841 effective-date fix), a different mechanism that does provably work. Full table and caveats in [eval-results.md](eval-results.md) “Hybrid vs pure-vector retrieval.”
...
**Identity, hosting, and integration.** Authentication and access control would move from the demo's lightweight scheme to enterprise SSO with role-based access to corpus partitions; some agencies operate with corpora segmented by clearance level. Hosting would need to satisfy FedRAMP requirements at the appropriate impact level, which constrains both the cloud provider choice and the LLM provider choice. The corpus would need to be tuned to agency-specific terminology and ontologies — FERC's vocabulary is a starting point, but a deployment for a specific utility regulator or DOE office would require additional fine-tuning or prompt-level customization. Human-in-the-loop review would be required for any output that could enter a compliance record or regulatory filing; the system would surface its draft answers into a review queue rather than returning them directly. Integration with existing case management, document management, or compliance workflow systems would be the work that determines whether the tool is actually adopted.

The remaining items are derived from concrete things this build surfaced — failed experiments, parser limitations, deploy-time issues. Naming them with specifics is part of the deliverable:

**Verification architecture: keep the LLM judge, don't replace it with structural checks.** The build included an experiment (v4, in [eval-results.md §"What didn't work"](eval-results.md)) replacing the runtime LLM-as-judge with structural verification: forcing the synthesizer to emit `(claim, chunk_id, supporting_quote)` triples via Anthropic tool use, then substring-checking each quote against its chunk. By construction, citation drift toward topical chunks should have become impossible. Instead, citation faithfulness *dropped* from 91.4% to 84.1% — the substring check is too permissive, accepting any quote that appears anywhere in the chunk even when the claim paraphrases beyond what the quote supports. The LLM judge correctly identifies this case; the substring check cannot. **Implication for production:** structural verification has appeal but isn't the right primary check; the LLM judge stays load-bearing. Quote verification could still be useful as a fast pre-filter ahead of the judge in a future iteration ([`apps/api/src/regrag_api/verification/quotes.py`](../apps/api/src/regrag_api/verification/quotes.py) is preserved in the repo for that purpose).

**Corpus discovery needs a real ingestion pipeline.** FERC's eLibrary is an Angular SPA with an undocumented JSON API; it can't be scraped with simple HTTP. The current corpus was hand-curated by guess-and-HEAD-checking PDF URLs at `www.ferc.gov/sites/default/files/{YYYY-MM}/...`, which works for a fixed seed list but doesn't extend. A production pipeline needs either Playwright driving the eLibrary SPA or — more pragmatically — a weekly scrape of `www.ferc.gov/news-events/news/decisions-notices` to detect new orders. The seed corpus can be expanded by hand as needed; the *ongoing* discovery is the production work.

**Parser fidelity is heterogeneous across PDF formats.** The current chunker handles two FERC PDF layouts well (eLibrary "Unofficial PDF" format, used for Orders 2222 / 841 / 2023 / etc.; Federal Register format with "FR Doc" headers). It collapses Order 872's PDF to ~2 substantive sections — the layout is different enough that the section-aware chunker misses paragraph boundaries. The fix is a fallback parser strategy: detect the format on a sample page and route to the appropriate chunker variant. Production deployments would also want OCR fallback for scanned legacy orders (pre-2000) that aren't text PDFs.

**Cold-start UX needs always-on infrastructure.** Railway's free-tier container sleeps after idle, which made first-request latency 30–60 seconds for any visitor who landed on a cold start. The demo mitigates this with a cron warmer (GitHub Actions pinging `/health` every 5 minutes) and a frontend pre-warm with an honest "warming up" hint, but the right answer at production scale is always-on infrastructure with autoscaling — measured in dollars per month, not engineering effort.

**Refusal calibration is the single biggest remaining gap.** The eval surfaces three borderline OOS questions where the LLM judge is too lenient: it constructs a hedged answer from tangentially-related chunks instead of refusing. For a regulated-domain deployment where "I don't know" is much better than "here is a confident but unsupported answer," this is the load-bearing failure mode. Production options: a stricter refusal-classifier as a separate stage (Haiku scoring "is this answerable from these chunks: yes/no" before synthesis runs), tighter pre-generation cosine thresholds tuned per persona, or human review of any refusal-borderline case. None of these are research problems — they're calibration work against a larger eval set than the 40 questions here.

**Continuous corpus updates need versioning the audit can replay.** Continuous corpus updates would need to handle versioning so that historical queries can be replayed against the corpus as it existed at the time. The current schema captures this by snapshotting `retrieved_chunks` (full JSONB blob) into the audit log on every query rather than storing chunk IDs that might be deleted on re-chunk. Production would extend this with a `corpus_snapshot_id` field tying queries to a specific corpus version, plus the operational tooling to maintain old snapshots — the kind of work that's invisible until you need it for a regulatory audit.

None of these are research problems. They are the work that turns a demonstration into a delivered solution, and naming them with specifics — derived from a real build, not a planning exercise — is part of the deliverable.
...
- **Evaluation results:** [docs/eval-results.md](eval-results.md) — current run (v6) is 95.8% retrieval recall / 92.5% refusal accuracy (77.8% precision, 87.5% recall) / 94.8% citation faithfulness across 40 questions on the 17-doc corpus. Includes per-persona breakdown, the v5→v6 delta with two honestly-accounted wrinkles, baseline comparison (vs vanilla RAG), and a "what didn't work" section on the structured-output verification experiment.
